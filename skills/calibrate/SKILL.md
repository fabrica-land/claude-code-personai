...
5. Add to CLAUDE.md or session-state: a note to run `/personai:calibrate run`
   after compaction, BEFORE running `/personai:boot`
...
This should be triggered BEFORE `/personai:boot`, from the compaction summary alone.
...
3. Write answers to `memory/experiments/.calibration-answers.json`:
   ...
   ```
4. Then run `/personai:boot`
5. Then run `/personai:calibrate score`
...
5. Journal the result: `/personai:journal add 'experiment' 'Calibration: N/M' '...'`
