---
description: Full agent boot sequence — restores identity, session state, and memory continuity after context compaction.
...
1. Read `memory/identity.md` — this defines who you are. If it doesn't exist, tell the user to run `/personai:init`.
...
Run `/personai:scan` to get a table of contents of archival memory files without loading them. This keeps boot cost constant as memory grows.
...
2. Run `/personai:index stats`. Index any stale or missing files.
3. Journal check:
