...
Search the semantic index built by `/personai:index`. Finds memory files by
...
- If no results, the index may need updating — run `/personai:index scan`
- After finding candidates, deep-read the top results
- Log misses — they help you improve keyword coverage over time

## Deeper Search

For semantic similarity search (beyond keyword matching), use `/personai:enrich`.
