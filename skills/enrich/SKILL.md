...
optional Sonnet-based relevance filtering. Use this when `/personai:search`
...
- **`/personai:search`** — Fast keyword lookups. Use for simple, specific queries.
- **`/personai:enrich`** — Semantic similarity + optional LLM filtering. Use when
...
- **`/personai:associate`** — Lightweight keyword-only associations, no vector, no LLM.
